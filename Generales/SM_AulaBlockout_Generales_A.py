...
# Aula de Estudios Generales (ARC-GEN-005), cuarto piso.
# (Proyecto_Memoria_Definitivo_3_Dias_Terror_Intenso.txt, seccion 10.2:
# "Aulas: 8 x 10 m"; altura de piso academico ~4.5 m, igual que Vestibulo,
# Soda y Escaleras de este mismo edificio.)
# Sala aislada: los pisos 1-3 y el pasillo del piso 4 todavia no tienen
# blockout, asi que se modela con origen local propio (0,0,0) en vez de
# encadenarse a las coordenadas del vestibulo/soda del piso 1. Si se
# ensambla el edificio completo mas adelante, el offset vertical
# aproximado de este piso seria Z = 3 * WALL_H = 13.5 m.
# Puerta unica hacia el futuro pasillo del piso 4; el resto de paredes
# queda cerrado por ahora, igual que la pared trasera de la Soda.
...
DOOR_W = 1.20  # puerta interior, ancho recomendado de la guia de dimensiones
...
front_side_w = (ROOM_W - DOOR_W) / 2.0
...
add_box("Wall_Front_R", front_side_w, WALL_THICK, WALL_H,
        (half_w - front_side_w / 2.0, -half_d + WALL_THICK / 2.0, WALL_H / 2.0))
add_box("Wall_Front_Header", DOOR_W, WALL_THICK, WALL_H - 2.6,
        (0.0, -half_d + WALL_THICK / 2.0, WALL_H - (WALL_H - 2.6) / 2.0))

add_box("Wall_Back", ROOM_W, WALL_THICK, WALL_H, (0.0, half_d - WALL_THICK / 2.0, WALL_H / 2.0))
...
mesh_obj.name = "SM_AulaBlockout_Generales_A"
mesh_obj.data.name = "SM_AulaBlockout_Generales_A_Mesh"
...
mat_floor = bpy.data.materials.new(name="M_Floor_AulaBlockout")
...
mat_wall = bpy.data.materials.new(name="M_Wall_AulaBlockout")
...
col_floor.name = "UCX_SM_AulaBlockout_Generales_A_00"
...
bpy.ops.wm.save_as_mainfile(
    filepath="C:\\Users\\jeffa\\Desktop\\Proyecto\\Modelos-3D\\Generales\\SM_AulaBlockout_Generales_A.blend")
